...
#### Medication EncounterNumber must match EnteredAt facility

Each medication's `<EncounterNumber>` should reference an encounter at the same facility as its `<EnteredAt>`. Set `<EncounterNumber>` to the encounter at which the medication was ordered or first documented.
