...
6. Log all changes to `auto_qa_changelog.jsonl`

**Usage:**
```
/auto-qa --template fl_demo.template.json --output population-fl_1000
/auto-qa --template fl_demo.template.json --output population-fl_1000 --count 1000
/auto-qa --template fl_demo.template.json --output population-fl_1000 --review-only
/auto-qa --template fl_demo.template.json --output population-fl_1000 --model gpt-4o-mini
```
...
- `--template <file>` (required) — template JSON file, e.g. `fl_demo.template.json`
- `--output <dir>` (required) — population output directory, e.g. `population-fl_1000`
...
Set `MAX_ROUNDS=5` (never exceed 5 fix-and-regenerate cycles). Set `CHANGELOG=auto_qa_changelog.jsonl`.
...
- `fix_target == "template"` → edit the template JSON file only.
- `fix_target == "generator"` → edit `generate_population.py` only.
- `fix_target == "both"` → fix both.

**How to fix:**

1. Read the relevant section of `generate_population.py` or the template JSON file using the Read tool (use a specific line range if you know it).
...
4. After each Python edit, verify syntax:
   ```bash
   python3 -m py_compile generate_population.py
   ```
...
- `generate_population.py` must remain pure Python with **no LLM calls** — never add imports of `openai`, `anthropic`, or any AI/LLM library.
...
**When an issue requires deeper investigation:** Use the Bash tool to grep for the relevant variable, function, or ICD prefix in `generate_population.py` before editing.
...
The changelog is `auto_qa_changelog.jsonl` in the **working directory** (not inside the output directory). Each line is a JSON object. Append (do not overwrite) using the Bash tool:

```bash
echo '<json>' >> auto_qa_changelog.jsonl
```
...
```json
{
  ...
  "files_changed": ["generate_population.py"]
}
```

The `auto_qa.py` script already logs `review_complete` events; you only need to log `fix_applied` events here.
...
1. Read `auto_qa_changelog.jsonl` and count the `fix_applied` entries.
2. Report:
   ```
   ── Auto-QA Complete ────────────────────────────────────
   ...
   Rounds:       N
   Fixes applied: N
   Changelog:    auto_qa_changelog.jsonl
...
- `OPENAI_API_KEY` must be set in the environment. If `auto_qa.py` exits 2 with a key error, remind the user to set it: `export OPENAI_API_KEY=sk-proj-...`
